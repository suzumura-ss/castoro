...
describe Castoro::Protocol::Command::Finalize do
  it 'should not be able to create an instance, because raise RuntimeError "Nil cannot be set for host."' do
    Proc.new {
      Castoro::Protocol::Command::Finalize.new "1.2.3", nil, "/host/1.2.."
  ...
  it 'should not be able to create an instance, because raise RuntimeError "Nil cannot be set for path."' do
    Proc.new {
      Castoro::Protocol::Command::Finalize.new "1.2.3", "host100", nil
    }.should raise_error(RuntimeError, "Nil cannot be set for path.")
  end

  context 'when initialize, argument for basket set "1.2.3", argument for host set "host100", argument for path set {"length" => "12345", "class" => 1}' do
    it "should be able to create an instance of finalize command." do
      Castoro::Protocol::Command::Finalize.new("1.2.3", "host100", "/path/1.2.3").should be_kind_of(Castoro::Protocol::Command::Finalize)
    end

    context "when initialized" do
      before do
        @command = Castoro::Protocol::Command::Finalize.new("1.2.3", "host100", "/path/1.2.3")
      end
      ...
      it "should be able to use #to_s." do
        JSON.parse(@command.to_s).should ==
          JSON.parse('["1.1","C","FINALIZE",{"basket":"1.2.3","host":"host100","path":"/path/1.2.3"}]' + "\r\n")
      end
...
describe Castoro::Protocol::Response::Finalize do
  context 'when initialize, argument for error set nil, argument for basket set "1.2.3"' do
    it 'should be able to create an instance of finalize response.' do
      Castoro::Protocol::Response::Finalize.new(nil, "1.2.3").should be_kind_of(Castoro::Protocol::Response::Finalize)
    end

    context "when initialized" do
      before do
        @response = Castoro::Protocol::Response::Finalize.new nil, "1.2.3"
      end
      ...
      it 'should be able to use #to_s.' do
        JSON.parse(@response.to_s).should ==
          JSON.parse('["1.1","R","FINALIZE",{"basket":"1.2.3"}]' + "\r\n")
      end
    end
  end
  ...
  context 'when initialize, argument for error set "Unexpected error!", argument for basket set "1.2.3"' do
    it 'should be able to create an instance of finalize error response.' do
      Castoro::Protocol::Response::Finalize.new("Unexpected error!", "1.2.3").should be_kind_of(Castoro::Protocol::Response::Finalize)
    end

    context "when initialized" do
      before do
        @response = Castoro::Protocol::Response::Finalize.new "Unexpected error!", "1.2.3"
      end
